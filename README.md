...
> A hands-on catalogue of **26 advanced relational database design patterns**, implemented as a single SQL Server (`OnlineStore`) schema that **evolves over time** — from a naive, anti-pattern-ridden first draft into a normalized, partitioned, temporal, secured and performance-tuned production model.
...
Each row links to the **runnable SQL** and a short **write-up**. The "What it solves" column is a one-line summary of the problem each pattern addresses.
...
Code and documentation © Michał Panasiuk, released under the [MIT License](LICENSE). Originally authored for the _Advanced Database Design Patterns in Action_ Udemy course.
